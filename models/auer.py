...
class AuerSubgaussianModel:
    ...
    def update(self):
        ...
        for design in self.design_samples:
            ...
            # Original beta
            t1 = np.log((4 * self.design_count * self.output_dim * ni**2) / self.delta)
            t2 = 1

            # # Empirical beta
            # v_hat = np.var(design, axis=0) if len(design) > 0 else self.noise_var
            # v_prime = 1
            ...
            beta = np.sqrt((2 * t1 * t2) / ni) * np.sqrt(self.noise_var)
            
            self.variances.append(beta / self.conf_contraction)
